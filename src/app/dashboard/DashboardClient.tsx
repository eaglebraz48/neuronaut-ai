...
export default function DashboardClient() {
  ...
  const situationHref = `/profile?lang=${lang}`;
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 24 }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        >
          ...
          <div style={{ marginTop: 6, fontSize: 12, opacity: 0.75 }}>
            {L.guest} •{' '}
            <Link href={`/sign-in?lang=${lang}`} style={{ textDecoration: 'underline' }}>
              {L.signin}
            </Link>
          </div>
        </div>
      ...
      <p style={{ marginBottom: 32, opacity: 0.85 }}>
        {L.welcomeFixed} — {L.guest}
      </p>

      <div style={{ display: 'grid', gap: 24 }}>
        <section style={cardStyle}>
          <h2>{L.situationTitle}</h2>
          <p>{L.situationDesc}</p>
          <Link href={situationHref} style={btnStyle}>
            {L.situationBtn}
          </Link>
        </section>

        <section style={cardStyle}>
          <h2>{L.snapshotTitle}</h2>
          <p>{L.snapshotDesc}</p>
          <Link href={`/snapshot?lang=${lang}`} style={btnStyle}>
            {L.snapshotBtn}
          </Link>
        </section>
        ...
        <section style={cardStyle}>
          <h2>{L.pathsTitle}</h2>
          <p>{L.pathsDesc}</p>
          <Link href={`/paths?lang=${lang}`} style={btnStyle}>
            {L.pathsBtn}
          </Link>
        </section>
      </div>
